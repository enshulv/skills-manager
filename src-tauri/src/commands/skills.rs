...
pub async fn check_all_skill_updates(
    ...
    tauri::async_runtime::spawn_blocking(move || {
        ...
        // No central-repo lock: Phase A already did every network read, and the
        // remaining work only writes skill status columns, which SQLite
        // serialises on its own busy_timeout. Feeding the prefetched revision in
        // keeps this loop off the network entirely.
        let mut failed = Vec::new();
        for skill in &skills {
